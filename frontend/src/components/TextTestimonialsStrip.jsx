...
const ICONS = { google: GoogleIcon, retention: UsersIcon, trust: HeartIcon, dna: DnaIcon };

/* ── Single Trust Badge (Soulful compact design) ── */
const TrustBadge = ({ card, delay = 0 }) => {
  const IconComp = ICONS[card.icon] || HeartIcon;
  const valStyle = applySectionStyle(card.value_style, {
    fontFamily: "'Cinzel', serif", fontSize: '1.4rem', fontWeight: 700, color: GOLD, lineHeight: 1,
  });
  ...
  const descStyle = applySectionStyle(card.description_style, {
    fontFamily: "'Lato', sans-serif", fontSize: '0.65rem', color: '#999', lineHeight: 1.5, fontWeight: 300,
  });

  return (
    <div
      data-testid={`trust-badge-${card.icon}`}
    ...
    >
      ...
      <p style={lblStyle} className="mt-1">{card.label}</p>
      <p style={descStyle} className="mt-1 max-w-[140px]">{card.description}</p>
    </div>
  );
};
...
const TextTestimonialsStrip = ({ sectionConfig }) => {
  ...
  useEffect(() => {
    Promise.all([
      ...
      // Load trust cards from homepage_sections config
  ...
  const leftCards = trustCards.slice(0, 2);
  const rightCards = trustCards.slice(2, 4);
    ...
    <section id="text-testimonials" data-testid="text-testimonials-section" className="relative overflow-hidden" style={{ padding: '52px 0 40px' }}>
      ...
      <div className="relative z-10">
        ...
        <div className={CONTAINER}>
          {/* Desktop: 3-column layout — badges | quote | badges */}
          <div className="flex items-center gap-6 lg:gap-10 max-w-6xl mx-auto">

            {/* Left trust badges — hidden on mobile */}
            <div className="hidden lg:flex flex-col gap-8 flex-shrink-0 w-[170px]" data-testid="trust-badges-left">
              {leftCards.map((card, i) => <TrustBadge key={i} card={card} delay={i * 150} />)}
            </div>

            {/* Decorative vertical line */}
            <div className="hidden lg:block w-px self-stretch" style={{ background: `linear-gradient(180deg, transparent, ${GOLD}30, transparent)` }} />
            ...
            <div className="flex-1 min-w-0">
              ...
              >
                ...
                <div className="mb-3 flex justify-center" aria-hidden="true">
                  <svg width="26" height="18" viewBox="0 0 36 28" fill="none" style={{ opacity: 0.18 }}>
                ...
                <blockquote
                  ...
                  style={{
                    ...
                    fontSize: `clamp(0.95rem, 2.2vw, ${quoteSize})`,
                    color: quoteColor,
                    fontStyle: quoteItalic ? 'italic' : 'normal',
                    fontWeight: 400, lineHeight: 1.9, letterSpacing: '0.01em', marginBottom: '18px',
                  }}
                >
                  {q.quote}
                </blockquote>
            ...
            <div className="hidden lg:flex flex-col gap-8 flex-shrink-0 w-[170px]" data-testid="trust-badges-right">
              {rightCards.map((card, i) => <TrustBadge key={i} card={card} delay={(i + 2) * 150} />)}
            </div>
          </div>

          {/* Mobile: Trust badges in a 2x2 grid below the quote */}
          <div className="lg:hidden grid grid-cols-2 gap-6 mt-8 max-w-sm mx-auto" data-testid="trust-badges-mobile">
            {trustCards.map((card, i) => <TrustBadge key={i} card={card} />)}
          </div>
        </div>
