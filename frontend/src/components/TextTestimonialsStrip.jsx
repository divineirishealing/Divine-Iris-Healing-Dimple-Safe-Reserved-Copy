          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 max-w-5xl mx-auto mb-10" data-testid="trust-strip">
            {trustCards.map((card, i) => {
              const valStyle = applySectionStyle(card.value_style, {
                fontFamily: "'Cinzel', serif", fontSize: '1.5rem', fontWeight: 700, color: '#fff', lineHeight: 1.1,
              });
              ...
              return (
                <div key={i} className="flex flex-col items-center text-center group" data-testid={`trust-item-${i}`}>
                  <div
                    className="w-24 h-24 rounded-full flex flex-col items-center justify-center mb-3 group-hover:scale-105 transition-transform duration-300 shadow-md"
                    style={{ background: `linear-gradient(135deg, ${GOLD}, #c5972a)` }}
                  >
                    <div className="flex items-center gap-1 mb-0.5">
                      {card.icon === 'google' && <svg width="12" height="12" viewBox="0 0 24 24"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#fff"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#fff"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#fff"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#fff"/></svg>}
                      {card.icon === 'google' && <div className="flex gap-px">{[...Array(5)].map((_, j) => <svg key={j} width="7" height="7" viewBox="0 0 24 24" fill="#fff" stroke="none"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" /></svg>)}</div>}
                    </div>
                    {card.value && <p style={valStyle}>{card.value}</p>}
                  </div>
                  <p style={lblStyle} className="mt-1 mb-1">{card.label}</p>
                  {card.description && <p style={descStyle} className="max-w-[180px]">{card.description}</p>}
                </div>
              );
            })}
          </div>
