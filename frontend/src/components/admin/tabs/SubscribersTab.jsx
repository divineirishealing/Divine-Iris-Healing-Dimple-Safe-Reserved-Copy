...
import {
  ...
  CreditCard, Calendar, Plus, X, Save, Edit2, Trash2, Check, CheckCircle
} from 'lucide-react';
...
const SubscriberForm = ({ initial, onSave, onCancel, saving }) => {
  ...
  return (
    <div className="bg-white border rounded-lg shadow-sm p-5 space-y-5" data-testid="subscriber-form">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div><Label className="text-xs">Start Date</Label><Input type="date" value={f.start_date} onChange={e => set('start_date', e.target.value)} /></div>
          <div><Label className="text-xs">End Date</Label><Input type="date" value={f.end_date} onChange={e => set('end_date', e.target.value)} /></div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <div>
          ...
          <select value={f.currency} onChange={e => set('currency', e.target.value)} className="w-full border rounded-md px-2 py-2 text-sm">{CURRENCY_OPTIONS.map(c => <option key={c} value={c}>{c}</option>)}</select>
        </div>
        <div>
              ...
              <tbody>
                {f.emis.map((emi, idx) => (
                  <tr key={emi.number} className="border-t">
                    <td className="px-2 py-1.5 font-medium">{emi.number}</td>
                    <td className="px-2 py-1.5"><Input type="date" value={emi.due_date} onChange={e => updateEmi(idx, 'due_date', e.target.value)} className="h-7 text-xs" /></td>
                    <td className="px-2 py-1.5"><Input type="number" value={emi.amount} onChange={e => updateEmi(idx, 'amount', e.target.value)} className="h-7 text-xs text-right w-24 ml-auto" /></td>
                    <td className="px-2 py-1.5"><Input type="number" value={emi.remaining} onChange={e => updateEmi(idx, 'remaining', e.target.value)} className="h-7 text-xs text-right w-24 ml-auto" /></td>
      ...
      <div>
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 gap-2 mt-1">
          <div><Label className="text-[10px]">Carry Fwd</Label><Input type="number" value={f.sessions.carry_forward} onChange={e => setSess('carry_forward', parseInt(e.target.value) || 0)} className="h-8 text-xs" /></div>
          <div><Label className="text-[10px]">Current</Label><Input type="number" value={f.sessions.current} onChange={e => setSess('current', parseInt(e.target.value) || 0)} className="h-8 text-xs" /></div>
          <div><Label className="text-[10px]">Total</Label><Input type="number" value={f.sessions.total || (f.sessions.carry_forward + f.sessions.current)} onChange={e => setSess('total', parseInt(e.target.value) || 0)} className="h-8 text-xs" /></div>
          <div><Label className="text-[10px]">Availed</Label><Input type="number" value={f.sessions.availed} onChange={e => setSess('availed', parseInt(e.target.value) || 0)} className="h-8 text-xs" /></div>
          <div><Label className="text-[10px]">Yet to Avail</Label><Input type="number" value={f.sessions.yet_to_avail} onChange={e => setSess('yet_to_avail', parseInt(e.target.value) || 0)} className="h-8 text-xs" /></div>
          <div><Label className="text-[10px]">Due</Label><Input type="number" value={f.sessions.due} onChange={e => setSess('due', parseInt(e.target.value) || 0)} className="h-8 text-xs" /></div>
        </div>
        {/* Scheduled Dates */}
          ...
          <div className="flex flex-wrap gap-1 mt-1">
            {f.sessions.scheduled_dates.map((d, i) => (
              <span key={i} className="px-2 py-0.5 bg-amber-50 text-amber-700 rounded-full text-[10px] flex items-center gap-1">
                {d} <button onClick={() => setSess('scheduled_dates', f.sessions.scheduled_dates.filter((_, j) => j !== i))} className="hover:text-red-500"><X size={8} /></button>
              </span>
            ))}
          </div>
        )}
          ...
          <div className="flex flex-wrap gap-1 mt-1">
            {f.programs.map((p, i) => (
              <span key={i} className="px-2 py-0.5 bg-purple-50 text-purple-700 rounded-full text-[10px] flex items-center gap-1">
                {p} <button onClick={() => set('programs', f.programs.filter((_, j) => j !== i))} className="hover:text-red-500"><X size={8} /></button>
              </span>
            ))}
          </div>
        )}
...
/* ─── SUBSCRIBER ROW (with quick actions) ─── */
  ...
  return (
    <>
      <tr className="border-b hover:bg-gray-50 text-xs" data-testid={`subscriber-row-${s.id}`}>
        ...
        <td className="px-3 py-2 font-medium truncate max-w-[180px]">{sub.annual_program}</td>
        ...
        <td className="px-3 py-2 text-center">
          <button onClick={() => onEdit(s)} className="text-[#5D3FD3] hover:underline text-[10px] font-semibold" data-testid={`edit-btn-${s.id}`}>
            <Edit2 size={12} />
          </button>
        </td>
      </tr>
        ...
        <tr>
          <td colSpan={9} className="bg-gray-50 px-6 py-4 border-b">
            <div className="grid md:grid-cols-2 gap-6">
                ...
                {emis.length === 0 ? (
                  ...
                  <table className="w-full text-[11px]">
                    <thead><tr className="text-gray-400 border-b">
                      <th className="text-left py-1">#</th><th className="text-left py-1">Due</th>
                      <th className="text-right py-1">Amt</th><th className="text-right py-1">Rem</th>
                      <th className="text-center py-1">Status</th><th className="py-1"></th>
                    </tr></thead>
                    <tbody>
                      {emis.map(e => (
                        <tr key={e.number} className="border-b border-gray-100">
                          ...
                          <td className="py-1 text-center">
                            {e.status !== 'paid' && (
                              <button
                                onClick={() => markEmiPaid(e.number)}
                                disabled={markingEmi === e.number}
              ...
              {/* Sessions & Programs + Quick Actions */}
              <div>
                <h4 className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-2 flex items-center gap-1">
                ...
                <div className="grid grid-cols-3 gap-2 text-[11px] mb-3">
                  <div className="bg-white p-2 rounded border text-center">
                    <div className="text-gray-400">Carry Fwd</div><div className="font-bold text-gray-900">{sess.carry_forward || 0}</div>
                  </div>
                  <div className="bg-white p-2 rounded border text-center">
                    <div className="text-gray-400">Current</div><div className="font-bold text-gray-900">{sess.current || 0}</div>
                  </div>
                  <div className="bg-white p-2 rounded border text-center">
                    <div className="text-gray-400">Due</div><div className="font-bold text-red-600">{sess.due || 0}</div>
                  </div>
                </div>
                {/* Quick: +1 Session */}
                ...
                {sub.programs?.length > 0 && (
                  <div className="mb-2">
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {sub.programs.map((p, i) => <span key={i} className="px-2 py-0.5 bg-purple-50 text-purple-700 rounded-full text-[10px] font-medium">{p}</span>)}
                    </div>
                  </div>
                )}
                {sess.scheduled_dates?.length > 0 && (
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {sess.scheduled_dates.map((d, i) => <span key={i} className="px-2 py-0.5 bg-amber-50 text-amber-700 rounded-full text-[10px]">{d}</span>)}
                    </div>
                  </div>
                )}
              </div>
...
const SubscribersTab = () => {
  ...
  const handleEdit = (s) => {
    ...
    setEditTarget(s);
    setShowForm(true);
    // Form will be pre-filled via initial prop
  };
  ...
  const formInitial = editTarget ? {
    ...
    start_date: editTarget.subscription?.start_date || '',
    end_date: editTarget.subscription?.end_date || '',
    total_fee: editTarget.subscription?.total_fee || 0,
    currency: editTarget.subscription?.currency || 'INR',
    payment_mode: editTarget.subscription?.payment_mode || 'No EMI',
    num_emis: editTarget.subscription?.num_emis || 0,
    emis: editTarget.subscription?.emis || [],
    programs: editTarget.subscription?.programs || [],
    bi_annual_download: editTarget.subscription?.bi_annual_download || 0,
    quarterly_releases: editTarget.subscription?.quarterly_releases || 0,
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white p-4 rounded-lg border shadow-sm">
        ...
        {uploadStats && (
          <div className="mt-2 p-2 bg-green-50 rounded border border-green-200 text-xs">
            Created: {uploadStats.created}, Updated: {uploadStats.updated}
      ...
      <div className="bg-white rounded-lg border shadow-sm overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="p-8 text-center text-sm text-gray-400 italic">No subscribers yet. Add one or upload Excel.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full min-w-[800px]">
              ...
              <tbody>
                {subscribers.map(s => <SubscriberRow key={s.id} s={s} onRefresh={fetchSubscribers} onEdit={handleEdit} />)}
              </tbody>
            </table>
          </div>
        )}
